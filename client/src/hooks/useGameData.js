...
import { useRandomAyahsForGame, useRandomSurahsForGame, useSurahs } from './useQuranData.js';
...
export function useIdentifySurahData() {
  ...
  const generateOptionsForAyah = useCallback((ayah, surahs) => {
  ...
  const initializeQuestion = useCallback((ayah, surahs) => {
    setCurrentAyah(ayah);
...
export function useSurahOrderingData() {
  ...
  const shuffleSurahs = useCallback((surahList) => {
  ...
  const handleMoveSurah = useCallback((dragIndex, hoverIndex) => {
  ...
  const initializeSurahs = useCallback((provided) => {
