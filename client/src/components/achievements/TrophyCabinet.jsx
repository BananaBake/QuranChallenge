...
export function Trophy({ achievement, highlighted = false }) {
  return (
    <motion.div
...
export function TrophyCabinet({ achievements, onSelect, selectedId }) {
  const sortedAchievements = [...achievements].sort((a, b) => {
    if (a.unlocked && !b.unlocked) return -1;
    if (!a.unlocked && b.unlocked) return 1;
    if (a.progress && b.progress) {
      // Ensure goal is not 0 to prevent division by zero
      const goalA = a.goal || 1;
      const goalB = b.goal || 1;
      return (b.progress / goalB) - (a.progress / goalA);
    }
    return 0;
  });
    ...
    >
      ...
      <div className="grid grid-cols-3 gap-4 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6">
        {sortedAchievements.map(achievement => (
          <div key={achievement.id} onClick={() => onSelect?.(achievement)} className="cursor-pointer">
            <Trophy 
...
export function TrophyDetails({ achievement, onClose }) {
  return (
    <motion.div
        ...
        <div className="bg-gray-50 p-3 rounded-lg">
          ...
          <div className="w-full h-3 bg-gray-200 rounded-full overflow-hidden">
            <div 
              ...
              style={{ width: `${Math.min(100, (achievement.progress / (achievement.goal || 1)) * 100)}%` }} // Ensure goal is not 0
            ></div>
          </div>
        </div>
      )}
